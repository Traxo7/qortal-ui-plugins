...
`);const Ml=/^((?!chrome|android).)*safari/i.test(navigator.userAgent),Bl=navigator.userAgent.toLowerCase().indexOf("firefox")>-1;if(Ml||Bl){const e=document.createElement("style");e.textContent="\n    [scrolling][safari] #outerscroller,\n    [scrolling][firefox] #outerscroller {\n      pointer-events: auto;\n    }\n\n    [ios] #outerscroller {\n      pointer-events: auto;\n      z-index: -3;\n    }\n\n    [ios][scrolling] #outerscroller {\n      z-index: 0;\n    }\n\n    [ios] [frozen] {\n      will-change: auto;\n    }\n  ",Fl.querySelector("template").content.appendChild(e)}Fl.register("vaadin-grid-styles");const Hl=(()=>{try{return document.createEvent("TouchEvent"),!0}catch(e){return!1}})();class Vl extends(Js(Ns(ml(hl(fl(cl(vl(xl(Al(yl(Rl(dl(gl(zl(pl(_l(kl(wl(ll))))))))))))))))))){static get template(){return ds`
...
</dom-module>`;document.head.appendChild(ud.content);const md=new he({type:"WINDOW",source:window.parent});window.customElements.define("trade-portal",class extends se{static get properties(){return{selectedAddress:{type:Object},config:{type:Object},qortBalance:{type:String},btcBalance:{type:String},sellBtnDisable:{type:Boolean},isSellLoading:{type:Boolean},isBuyLoading:{type:Boolean},buyBtnDisable:{type:Boolean},initialAmount:{type:Number},openOrders:{type:Array},historicTrades:{type:Array},myOpenOrders:{type:Array},myHistoricTrades:{type:Array},_openOrdersStorage:{type:Array},_myOpenOrdersStorage:{type:Array}}}static get styles(){return oe`
            * {
