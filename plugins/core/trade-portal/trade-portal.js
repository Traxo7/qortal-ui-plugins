...
`);const Kl=/^((?!chrome|android).)*safari/i.test(navigator.userAgent),Ql=navigator.userAgent.toLowerCase().indexOf("firefox")>-1;if(Kl||Ql){const e=document.createElement("style");e.textContent="\n    [scrolling][safari] #outerscroller,\n    [scrolling][firefox] #outerscroller {\n      pointer-events: auto;\n    }\n\n    [ios] #outerscroller {\n      pointer-events: auto;\n      z-index: -3;\n    }\n\n    [ios][scrolling] #outerscroller {\n      z-index: 0;\n    }\n\n    [ios] [frozen] {\n      will-change: auto;\n    }\n  ",Jl.querySelector("template").content.appendChild(e)}Jl.register("vaadin-grid-styles");const Zl=(()=>{try{return document.createEvent("TouchEvent"),!0}catch(e){return!1}})();class ed extends(dl(Ys(Tl(Cl(Il(wl(zl(Nl(Ll(Rl(ql(Al(Ol(jl(Sl(Pl(Ul(Dl(xl))))))))))))))))))){static get template(){return As`
...
`}static get is(){return"vaadin-text-field"}static get version(){return"2.6.2"}static get properties(){return{list:{type:String},pattern:{type:String},title:{type:String}}}}customElements.define(_d.is,_d);class gd extends class extends ws{}{static get template(){return As`
    <style>
      :host {
        display: inline-flex;
        max-width: 100%;
...
`}static get is(){return"vaadin-grid-filter"}static get properties(){return{path:String,value:{type:String,notify:!0},_connected:Boolean}}connectedCallback(){super.connectedCallback(),this._connected=!0}static get observers(){return["_filterChanged(path, value, _connected)"]}ready(){super.ready();const e=Ma(this).firstElementChild;e&&"filter"!==e.getAttribute("slot")&&(console.warn('Make sure you have assigned slot="filter" to the child elements of <vaadin-grid-filter>'),e.setAttribute("slot","filter"))}_filterChanged(e,t,i){void 0!==e&&void 0!==t&&i&&(void 0===this._previousValue&&""===t||(this._previousValue=t,this._debouncerFilterChanged=vo.debounce(this._debouncerFilterChanged,gn.after(200),()=>{this.dispatchEvent(new CustomEvent("filter-changed",{bubbles:!0}))})))}focus(){this.$.filter.focus()}}customElements.define(gd.is,gd);class bd extends Wl{static get template(){return As`
...
`}static get is(){return"vaadin-grid-filter-column"}static get properties(){return{path:String,header:String}}_prepareHeaderTemplate(){const e=this._prepareTemplatizer(this.$.headerTemplate);return e.templatizer.dataHost=this,e}_getHeader(e,t){return e||this._generateHeader(t)}}customElements.define(bd.is,bd);const yd=As`<dom-module id="material-checkbox" theme-for="vaadin-checkbox">
  <template>
    <style>
      :host {
        display: inline-block;
...
</dom-module>`;document.head.appendChild(yd.content);class vd extends Wl{static get template(){return As`
...
</dom-module>`;document.head.appendChild(xd.content);class Ad extends Wl{static get template(){return As`
    <template class="header" id="headerTemplate">
      <vaadin-grid-sorter path="[[path]]" direction="{{direction}}">[[_getHeader(header, path)]]</vaadin-grid-sorter>
    </template>
`}static get is(){return"vaadin-grid-sort-column"}static get properties(){return{path:String,direction:{type:String,notify:!0}}}_prepareHeaderTemplate(){const e=this._prepareTemplatizer(this.$.headerTemplate);return e.templatizer.dataHost=this,e}_getHeader(e,t){return e||this._generateHeader(t)}}customElements.define(Ad.is,Ad);const wd=As`<dom-module id="material-grid-tree-toggle" theme-for="vaadin-grid-tree-toggle">
  <template>
    <style>
      :host {
  ...
  <template>
    <style>
      ...
      :host([dir="rtl"]) #scroller:not([ios]) [part~="row"][dragstart]:not([dragstart=""])::after {
...
</dom-module>`;document.head.appendChild(Td.content);const Id=new he({type:"WINDOW",source:window.parent});window.customElements.define("trade-portal",class extends se{static get properties(){return{selectedAddress:{type:Object},config:{type:Object},qortBalance:{type:String},btcBalance:{type:String},sellBtnDisable:{type:Boolean},isSellLoading:{type:Boolean},isBuyLoading:{type:Boolean},buyBtnDisable:{type:Boolean},initialAmount:{type:Number},openOrders:{type:Array},historicTrades:{type:Array},myOrders:{type:Array},myHistoricTrades:{type:Array},tradeOffersSocketCounter:{type:Number},cancelBtnDisable:{type:Boolean}}}static get styles(){return oe`
            * {
                            ...
                            <div class="box">
                                ...
                                <div class="border-wrapper">
                                    <vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="openOrdersGrid" aria-label="Open Orders" .items="${this.openOrders}">
                                        ...
                                        <vaadin-grid-column resizable header="Total (BTC)" .renderer=${(e,t,i)=>{L(F`<span> ${i.item.btcAmount} </span>`,e)}}>
                                        </vaadin-grid-column>
                                    </vaadin-grid>
                                </div>
                            </div>
                            ...
                            <div class="box">
                                ...
                                <div class="border-wrapper">
                                    <vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="historicTradesGrid" aria-label="Historic Trades" .items="${this.historicTrades}">
                                        <vaadin-grid-column resizable header="Amount (QORT)" path="qortAmount"></vaadin-grid-column>
                                        <vaadin-grid-column resizable header="Price (BTC)" .renderer=${(e,t,i)=>{const r=this.round(parseFloat(i.item.btcAmount)/parseFloat(i.item.qortAmount));L(F`${r}`,e)}}>
                                </vaadin-grid-column>
                                        <vaadin-grid-column resizable header="Total (BTC)" .renderer=${(e,t,i)=>{L(F`<span> ${i.item.btcAmount} </span>`,e)}}>
                                        </vaadin-grid-column>
                                    </vaadin-grid>
                            ...
                            <div class="box">
                                ...
                                <div class="border-wrapper">
                                    <vaadin-grid theme="compact column-borders row-stripes wrap-cell-content" id="myOrdersGrid" aria-label="My Orders" .items="${this.myOrders}">
                                        ...
                                        <vaadin-grid-column resizable header="Status" .renderer=${(e,t,i)=>{L(F`<span id="${i.item.atAddress}"> ${i.item._tradeState} </span>`,e)}}>
                                        </vaadin-grid-column>
                                        <vaadin-grid-column resizable header="Price (BTC)" .renderer=${(e,t,i)=>{const r=this.round(parseFloat(i.item.bitcoinAmount)/parseFloat(i.item.qortAmount));L(F`${r}`,e)}}>
                                </vaadin-grid-column>
                                        <vaadin-grid-column resizable header="Amount (QORT)" path="qortAmount"></vaadin-grid-column>
                                        <vaadin-grid-column resizable header="Total (BTC)" path="bitcoinAmount"></vaadin-grid-column>
                                        <vaadin-grid-column resizable width="5rem" flex-grow="0" header="Action" .renderer=${(e,t,i)=>{L(F`${this.renderCancelButton(i.item)}`,e)}}></vaadin-grid-column>
                                    </vaadin-grid>
                                </div>
                            </div>
                        </div>
        ...
        `}firstUpdated(){this.updateBTCAccountBalance(),this.openOrdersGrid=this.shadowRoot.getElementById("openOrdersGrid"),this.openOrdersGrid.querySelector("#priceColumn").headerRenderer=function(e){e.innerHTML='<vaadin-grid-sorter path="priceBtc" direction="asc">Price (BTC)</vaadin-grid-sorter>'},this.myOrdersGrid=this.shadowRoot.getElementById("myOrdersGrid"),this.historicTradesGrid=this.shadowRoot.getElementById("historicTradesGrid"),this.myHistoricTradesGrid=this.shadowRoot.getElementById("myHistoricTradesGrid"),this.getOpenOrdersGrid(),window.addEventListener("contextmenu",e=>{e.preventDefault(),this._textMenu(e)}),window.addEventListener("click",()=>{Id.request("closeCopyTextMenu",null)}),window.onkeyup=e=>{27===e.keyCode&&Id.request("closeCopyTextMenu",null)},Id.ready().then(()=>{this.initSocket(),Id.subscribe("selected_address",async e=>{this.selectedAddress={},(e=JSON.parse(e))&&0!==Object.entries(e).length&&(this.selectedAddress=e,this.updateAccountBalance())}),Id.subscribe("config",e=>{this.config=JSON.parse(e)}),Id.subscribe("copy_menu_switch",async e=>{"false"===e&&0!==window.getSelection().toString().length&&this.clearSelection()})}),Id.imReady()}fillBuyForm(e){this.shadowRoot.getElementById("buyAmountInput").value=parseFloat(e.qortAmount),this.shadowRoot.getElementById("buyPriceInput").value=this.round(parseFloat(e.btcAmount)/parseFloat(e.qortAmount)),this.shadowRoot.getElementById("buyTotalInput").value=parseFloat(e.btcAmount),this.shadowRoot.getElementById("qortalAtAddress").value=e.qortalAtAddress,this.buyBtnDisable=!1}getOpenOrdersGrid(){const e=this.shadowRoot.querySelector("#openOrdersGrid");e.addEventListener("click",t=>{let i=e.getEventContext(t).item;void 0!==i&&i.qortalCreator!==this.selectedAddress.address&&this.fillBuyForm(i)},{passive:!0})}processOfferingTrade(e){const t={...e,priceBtc:this.round(parseFloat(e.btcAmount)/parseFloat(e.qortAmount))};0===this.openOrdersGrid.items.length?(()=>{this.openOrdersGrid.items.push(t),this.openOrdersGrid.clearCache()})():(()=>{this.openOrdersGrid.items.unshift(t),this.openOrdersGrid.clearCache()})()}processRedeemedTrade(e){(e.qortalCreator===this.selectedAddress.address||e.partnerQortalReceivingAddress===this.selectedAddress.address)&&(this.tradeOffersSocketCounter>1&&this.updateBTCAccountBalance(),this.myHistoricTradesGrid.items.unshift(e),this.myHistoricTradesGrid.clearCache()),this.historicTradesGrid.items.unshift(e),this.historicTradesGrid.clearCache()}processTradingTrade(e){this.tradeOffersSocketCounter>1&&this.updateBTCAccountBalance(),this.openOrdersGrid.items.forEach((t,i)=>{t.qortalAtAddress===e.qortalAtAddress&&(this.openOrdersGrid.items.splice(i,1),this.openOrdersGrid.clearCache())})}processRefundedTrade(e){e.qortalCreator===this.selectedAddress.address&&(this.tradeOffersSocketCounter>1&&this.updateBTCAccountBalance(),this.myHistoricTradesGrid.items.unshift(e),this.myHistoricTradesGrid.clearCache())}processCancelledTrade(e){e.qortalCreator===this.selectedAddress.address&&(this.tradeOffersSocketCounter>1&&this.updateBTCAccountBalance(),this.myHistoricTradesGrid.items.unshift(e),this.myHistoricTradesGrid.clearCache()),this.openOrdersGrid.items.forEach((t,i)=>{t.qortalAtAddress===e.qortalAtAddress&&(this.openOrdersGrid.items.splice(i,1),this.openOrdersGrid.clearCache())})}processTradeOffers(e){e.forEach(e=>{"OFFERING"==e.mode?this.processOfferingTrade(e):"REDEEMED"==e.mode?this.processRedeemedTrade(e):"TRADING"==e.mode?this.processTradingTrade(e):"REFUNDED"==e.mode?this.processRefundedTrade(e):"CANCELLED"==e.mode&&this.processCancelledTrade(e)})}processTradeBotStates(e){e.reverse(),e.forEach(e=>{e.creatorAddress===this.selectedAddress.address&&("BOB_WAITING_FOR_AT_CONFIRM"==e.tradeState?this.changeTradeBotState(e,"PENDING"):"BOB_WAITING_FOR_MESSAGE"==e.tradeState?this.changeTradeBotState(e,"LISTED"):"BOB_WAITING_FOR_P2SH_B"==e.tradeState||"BOB_WAITING_FOR_AT_REDEEM"==e.tradeState?this.changeTradeBotState(e,"TRADING"):"BOB_DONE"==e.tradeState||"BOB_REFUNDED"==e.tradeState?this.handleCompletedState(e):"ALICE_WAITING_FOR_P2SH_A"==e.tradeState?this.changeTradeBotState(e,"PENDING"):"ALICE_WAITING_FOR_AT_LOCK"==e.tradeState||"ALICE_WATCH_P2SH_B"==e.tradeState?this.changeTradeBotState(e,"TRADING"):"ALICE_DONE"==e.tradeState?this.handleCompletedState(e):"ALICE_REFUNDING_B"==e.tradeState||"ALICE_REFUNDING_A"==e.tradeState?this.changeTradeBotState(e,"REFUNDING"):"ALICE_REFUNDED"==e.tradeState&&this.handleCompletedState(e))})}changeTradeBotState(e,t){this.myOrdersGrid.loading=!0;const i={...e,_tradeState:t};this.myOrdersGrid.querySelector("#"+e.atAddress)?(()=>{this.myOrdersGrid.items.forEach((t,r)=>{t.atAddress===e.atAddress&&(this.myOrdersGrid.items.splice(r,1),this.myOrdersGrid.items.unshift(i),this.myOrdersGrid.clearCache())})})():(()=>{this.myOrdersGrid.loading=!1,this.myOrdersGrid.items.unshift(i),this.myOrdersGrid.clearCache()})()}handleCompletedState(e){this.myOrdersGrid.items.forEach((t,i)=>{t.atAddress===e.atAddress&&(this.myOrdersGrid.items.splice(i,1),this.myOrdersGrid.clearCache())})}initSocket(){const e=()=>{let e,t,r=window.parent.reduxStore.getState().app.nodeConfig.knownNodes[window.parent.reduxStore.getState().app.nodeConfig.node],n=r.domain+":"+r.port;t="https:"===window.parent.location.protocol?`wss://${n}/websockets/crosschain/tradeoffers?includeHistoric=true`:`ws://${n}/websockets/crosschain/tradeoffers?includeHistoric=true`;const o=new WebSocket(t);o.onopen=()=>{setTimeout(a,50),this.tradeOffersSocketCounter+=1,console.log("[TRADE-OFFERS-SOCKET] ==>: CONNECTED")},o.onmessage=e=>{this.processTradeOffers(JSON.parse(e.data))},o.onclose=()=>{clearTimeout(e),console.log("[TRADE-OFFERS-SOCKET] ==>: CLOSED"),i()},o.onerror=t=>{clearTimeout(e),console.log("[TRADE-OFFERS-SOCKET] ==>: "+t.type)};const a=()=>{o.send("ping"),e=setTimeout(a,295e3)}},t=()=>{let e,t,i=window.parent.reduxStore.getState().app.nodeConfig.knownNodes[window.parent.reduxStore.getState().app.nodeConfig.node],n=i.domain+":"+i.port;t="https:"===window.parent.location.protocol?`wss://${n}/websockets/crosschain/tradebot`:`ws://${n}/websockets/crosschain/tradebot`;const o=new WebSocket(t);o.onopen=()=>{setTimeout(a,50),console.log("[TRADEBOT-SOCKET] ==>: CONNECTED")},o.onmessage=e=>{this.processTradeBotStates(JSON.parse(e.data))},o.onclose=()=>{clearTimeout(e),console.log("[TRADEBOT-SOCKET] ==>: CLOSED"),r()},o.onerror=t=>{clearTimeout(e),console.log("[TRADEBOT-SOCKET] ==>: "+t.type)};const a=()=>{o.send("ping"),e=setTimeout(a,295e3)}},i=()=>{setTimeout(()=>e(),3e3)},r=()=>{setTimeout(()=>t(),3e3)};e(),t()}async sellAction(){this.isSellLoading=!0,this.sellBtnDisable=!0;const e=this.shadowRoot.getElementById("sellAmountInput").value,t=this.shadowRoot.getElementById("sellTotalInput").value,i=this.round(parseFloat(e)+.001),r=async()=>await Id.request("tradeBotCreateRequest",{creatorPublicKey:this.selectedAddress.base58PublicKey,qortAmount:parseFloat(e),fundingQortAmount:i,bitcoinAmount:parseFloat(t),tradeTimeout:10080,receivingAddress:this.selectedAddress.btcWallet.address}),n=e=>{!0===e?(this.isSellLoading=!1,this.sellBtnDisable=!1,this.shadowRoot.getElementById("sellAmountInput").value=this.initialAmount,this.shadowRoot.getElementById("sellPriceInput").value=this.initialAmount,this.shadowRoot.getElementById("sellTotalInput").value=this.initialAmount):!1===e?(this.isSellLoading=!1,this.sellBtnDisable=!1,Id.request("showSnackBar","Failed to Create Trade. Try again!")):(this.isSellLoading=!1,this.sellBtnDisable=!1,Id.request("showSnackBar","Failed to Create Trade. ERROR_CODE: "+e.message))};if(this.round(parseFloat(i)+parseFloat(.002))>parseFloat(this.qortBalance))return this.isSellLoading=!1,this.sellBtnDisable=!1,Id.request("showSnackBar","Insufficient Funds!"),!1;n(await r())}async buyAction(){this.isBuyLoading=!0,this.buyBtnDisable=!0;const e=this.shadowRoot.getElementById("qortalAtAddress").value;(e=>{!0===e?(this.isBuyLoading=!1,this.buyBtnDisable=!0,this.shadowRoot.getElementById("buyAmountInput").value=this.initialAmount,this.shadowRoot.getElementById("buyPriceInput").value=this.initialAmount,this.shadowRoot.getElementById("buyTotalInput").value=this.initialAmount,this.shadowRoot.getElementById("qortalAtAddress").value="",Id.request("showSnackBar","Buy Request Successful!")):!1===e?(this.isBuyLoading=!1,this.buyBtnDisable=!1,Id.request("showSnackBar","Failed to Create Trade. Try again!")):(this.isBuyLoading=!1,this.buyBtnDisable=!1,Id.request("showSnackBar","Failed to Create Trade. ERROR_CODE: "+e.message))})(await(async()=>await Id.request("tradeBotRespondRequest",{atAddress:e,xprv58:this.selectedAddress.btcWallet.derivedMasterPrivateKey,receivingAddress:this.selectedAddress.address}))())}async cancelAction(e){const t=this.shadowRoot.querySelector("mwc-button#"+e.atAddress);t.innerHTML="<paper-spinner-lite active></paper-spinner-lite>",this.cancelBtnDisable=!0;(e=>{!0===e?(t.remove(),this.cancelBtnDisable=!1,Id.request("showSnackBar","Trade Cancelling In Progress!")):!1===e?(t.innerHTML="CANCEL",this.cancelBtnDisable=!1,Id.request("showSnackBar","Failed to Cancel Trade. Try again!")):(t.innerHTML="CANCEL",this.cancelBtnDisable=!1,Id.request("showSnackBar","Failed to Cancel Trade. ERROR_CODE: "+e.message))})(await(async()=>await Id.request("deleteTradeOffer",{creatorPublicKey:this.selectedAddress.base58PublicKey,atAddress:e.atAddress}))())}updateAccountBalance(){clearTimeout(this.updateAccountBalanceTimeout),Id.request("apiCall",{url:"/addresses/balance/"+this.selectedAddress.address}).then(e=>{this.qortBalance=e,this.updateAccountBalanceTimeout=setTimeout(()=>this.updateAccountBalance(),1e4)})}updateBTCAccountBalance(){Id.request("apiCall",{url:"/crosschain/btc/walletbalance",method:"POST",body:window.parent.reduxStore.getState().app.selectedAddress.btcWallet.derivedMasterPrivateKey}).then(e=>{this.btcBalance=(Number(e)/1e8).toFixed(8)})}renderCancelButton(e){return"BOB_WAITING_FOR_MESSAGE"===e.tradeState?F`<mwc-button id="${e.atAddress}" ?disabled=${this.cancelBtnDisable} class="cancel" @click=${t=>this.cancelAction(e)}>CANCEL</mwc-button>`:""}_checkSellAmount(e){const t=e.target.value,i=e.target;if(0===t.length)this.isValidAmount=!1,this.sellBtnDisable=!0,e.target.blur(),e.target.focus(),e.target.invalid=!0,e.target.validationMessage="Invalid Amount!";else{const e=this.shadowRoot.getElementById("sellAmountInput").value,t=this.shadowRoot.getElementById("sellPriceInput").value;this.shadowRoot.getElementById("sellTotalInput").value=this.round(parseFloat(e)*parseFloat(t)),this.sellBtnDisable=!1}e.target.blur(),e.target.focus(),e.target.validityTransform=(e,t)=>{if(!0===e.includes("-"))return this.sellBtnDisable=!0,i.validationMessage="Invalid Amount!",{valid:!1};if(t.valid){const e=this.shadowRoot.getElementById("sellAmountInput").value,t=this.shadowRoot.getElementById("sellPriceInput").value;this.shadowRoot.getElementById("sellTotalInput").value=this.round(parseFloat(e)*parseFloat(t)),this.sellBtnDisable=!1}else if(!0===e.includes(".")){if(!(e.split(".")[1].length>8)){const e=this.shadowRoot.getElementById("sellAmountInput").value,t=this.shadowRoot.getElementById("sellPriceInput").value;return this.shadowRoot.getElementById("sellTotalInput").value=this.round(parseFloat(e)*parseFloat(t)),this.sellBtnDisable=!1,{valid:!0}}this.sellBtnDisable=!0,i.validationMessage="Invalid Amount!"}}}_checkBuyAmount(e){const t=e.target.value,i=e.target;0===t.length?(this.isValidAmount=!1,this.sellBtnDisable=!0,e.target.blur(),e.target.focus(),e.target.invalid=!0,e.target.validationMessage="Invalid Amount!"):this.buyBtnDisable=!1,e.target.blur(),e.target.focus(),e.target.validityTransform=(e,t)=>{if(!0===e.includes("-"))return this.buyBtnDisable=!0,i.validationMessage="Invalid Amount!",{valid:!1};if(t.valid)this.buyBtnDisable=!1;else if(!0===e.includes(".")){if(!(e.split(".")[1].length>8))return this.buyBtnDisable=!1,{valid:!0};this.buyBtnDisable=!0,i.validationMessage="Invalid Amount!"}}}clearSelection(){window.getSelection().removeAllRanges(),window.parent.getSelection().removeAllRanges()}_textMenu(e){const t=()=>{var e="";return void 0!==window.getSelection?e=window.getSelection().toString():void 0!==this.shadowRoot.selection&&"Text"==this.shadowRoot.selection.type&&(e=this.shadowRoot.selection.createRange().text),e};(()=>{let i=t();if(i&&"string"==typeof i){let t={selectedText:i,eventObject:{pageX:e.pageX,pageY:e.pageY,clientX:e.clientX,clientY:e.clientY},isFrame:!0};Id.request("openCopyTextMenu",t)}})()}clearBuyForm(){this.shadowRoot.getElementById("buyAmountInput").value=this.initialAmount,this.shadowRoot.getElementById("buyPriceInput").value=this.initialAmount,this.shadowRoot.getElementById("buyTotalInput").value=this.initialAmount,this.shadowRoot.getElementById("qortalAtAddress").value="",this.buyBtnDisable=!0}clearSellForm(){this.shadowRoot.getElementById("sellAmountInput").value=this.initialAmount,this.shadowRoot.getElementById("sellPriceInput").value=this.initialAmount,this.shadowRoot.getElementById("sellTotalInput").value=this.initialAmount}isEmptyArray(e){return!e||0===e.length}round(e){return(Math.round(1e8*parseFloat(e))/1e8).toFixed(8)}})}));
