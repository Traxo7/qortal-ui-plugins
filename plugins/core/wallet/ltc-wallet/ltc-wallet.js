...
`);const _l=/^((?!chrome|android).)*safari/i.test(navigator.userAgent),fl=navigator.userAgent.toLowerCase().indexOf("firefox")>-1;if(_l||fl){const e=document.createElement("style");e.textContent="\n    [scrolling][safari] #outerscroller,\n    [scrolling][firefox] #outerscroller {\n      pointer-events: auto;\n    }\n\n    [ios] #outerscroller {\n      pointer-events: auto;\n      z-index: -3;\n    }\n\n    [ios][scrolling] #outerscroller {\n      z-index: 0;\n    }\n\n    [ios] [frozen] {\n      will-change: auto;\n    }\n  ",ml.querySelector("template").content.appendChild(e)}ml.register("vaadin-grid-styles");const gl=(()=>{try{return document.createEvent("TouchEvent"),!0}catch(e){return!1}})();class bl extends(Ta(ha($a(Ga(qa(Va(Xa(Za(Qa(Ka(dl(Ba(Ya(cl(Ua(Wa(ll(el(Ha))))))))))))))))))){static get template(){return Bs`
  ...
  <template>
    <style>
      ...
      :host([dir="rtl"]) #scroller:not([ios]) [part~="row"][dragstart]:not([dragstart=""])::after {
        ...
        `}_textMenu(e){const t=()=>{var e="";return void 0!==window.getSelection?e=window.getSelection().toString():void 0!==this.shadowRoot.selection&&"Text"==this.shadowRoot.selection.type&&(e=this.shadowRoot.selection.createRange().text),e};(()=>{let i=t();if(i&&"string"==typeof i){let t={selectedText:i,eventObject:{pageX:e.pageX,pageY:e.pageY,clientX:e.clientX,clientY:e.clientY},isFrame:!0};fd.request("openCopyTextMenu",t)}})()}clearSelection(){window.getSelection().removeAllRanges(),window.parent.getSelection().removeAllRanges()}transactionItem(e){return`\n            <div class='transaction-item ${e.type}'>\n                <div class='transaction-item_details'>\n                    <h3>${e.name}</h3>\n                    <span class='details'>${e.category} ${e.ID} - ${e.date}</span>\n                </div>\n                <div class='transaction-item_amount'>\n                    <p class='amount'>${e.amount}</p>\n                </div>\n            </div>\n        `}firstUpdated(){this.defaultWallet=this.shadowRoot.querySelector(".currency-box.qort"),this.defaultWallet.classList.add("active"),this.selectedWallet={type:"qort",currencyBox:this.defaultWallet},this.currencyBoxes=this.shadowRoot.querySelectorAll(".currency-box"),this.transactionsDOM=this.shadowRoot.getElementById("transactions"),this.currencyBoxes.forEach(e=>{e.addEventListener("click",this.selectWallet)}),this.showWallet(this.selectedWallet),window.addEventListener("contextmenu",e=>{e.preventDefault(),this.isTextMenuOpen=!0,this._textMenu(e)}),window.addEventListener("click",()=>{this.isTextMenuOpen&&fd.request("closeCopyTextMenu",null)}),window.onkeyup=e=>{27===e.keyCode&&fd.request("closeCopyTextMenu",null)}}selectWallet(e){e.preventDefault(),this.classList.contains("active")||(console.log(this.attributes.type.value),this.selectedWallet={type:this.attributes.type.value,currencyBox:this},console.log(this.selectedWallet))}async showWallet(e){console.log(e),console.log(this.qortWallet),"qort"===e.type?(this.fetchQortTransactions(),this.fetchQortBalance()):"btc"===e.type||e.type}fetchQortTransactions(){this.transactions=[],fd.request("apiCall",{url:`/transactions/search?address=${this.qortWallet.address}&confirmationStatus=BOTH&limit=20&reverse=true`}).then(e=>{this.transactions=e})}fetchQortBalance(){fd.request("apiCall",{url:"/addresses/balance/"+this.qortWallet.address}).then(e=>{this.balance=e})}updateAccountTransactions(){fd.request("apiCall",{url:"/crosschain/ltc/wallettransactions",method:"POST",body:window.parent.reduxStore.getState().app.selectedAddress.ltcWallet._tDerivedmasterPublicKey}).then(e=>{this.transactions=e,console.log(this.transactions)})}updateAccountBalance(){fd.request("apiCall",{url:"/crosschain/ltc/walletbalance",method:"POST",body:window.parent.reduxStore.getState().app.selectedAddress.ltcWallet._tDerivedmasterPublicKey}).then(e=>{this.balance=(Number(e)/1e8).toFixed(8)}).catch(e=>{fd.request("showSnackBar","Failed to Fetch Balance. Try again!")})}showTransactionDetails(e,t){t.forEach(t=>{if(e.signature===t.signature){let i=e.creatorAddress===this.selectedLtcWallet.address?"OUT":"IN";this.selectedTransaction={...t,txnFlow:i},0!=this.selectedTransaction.signature.length&&this.shadowRoot.querySelector("#showTransactionDetailsDialog").show()}})}isEmptyArray(e){return!e||0===e.length}floor(e){return e=parseFloat(e),isNaN(e)?0:this._format(Math.floor(e))}decimals(e){return(e=parseFloat(e))%1>0?(e+"").split(".")[1]:"0"}sendOrRecieve(e){return e.sender==this.selectedLtcWallet._taddress}senderOrRecipient(e){return this.sendOrRecieve(e)?e.recipient:e.sender}txColor(e){return this.sendOrRecieve(e)?"red":"green"}subtract(e,t){return e-t}getConfirmations(e,t){return t-e+1}_format(e){return e.toLocaleString()}textColor(e){return"light"===e?"rgba(255,255,255,0.7)":"rgba(0,0,0,0.87)"}_unconfirmedClass(e){return e?"unconfirmed":""}})}));
