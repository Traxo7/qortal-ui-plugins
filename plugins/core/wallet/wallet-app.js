...
`);const _l=/^((?!chrome|android).)*safari/i.test(navigator.userAgent),fl=navigator.userAgent.toLowerCase().indexOf("firefox")>-1;if(_l||fl){const e=document.createElement("style");e.textContent="\n    [scrolling][safari] #outerscroller,\n    [scrolling][firefox] #outerscroller {\n      pointer-events: auto;\n    }\n\n    [ios] #outerscroller {\n      pointer-events: auto;\n      z-index: -3;\n    }\n\n    [ios][scrolling] #outerscroller {\n      z-index: 0;\n    }\n\n    [ios] [frozen] {\n      will-change: auto;\n    }\n  ",ml.querySelector("template").content.appendChild(e)}ml.register("vaadin-grid-styles");const gl=(()=>{try{return document.createEvent("TouchEvent"),!0}catch(e){return!1}})();class bl extends(Ta(ha($a(Ga(qa(Va(Xa(Za(Qa(Ka(dl(Ba(Wa(cl(Ua(Ya(ll(el(Ha))))))))))))))))))){static get template(){return Bs`
  ...
  <template>
    <style>
      ...
      :host([dir="rtl"]) #scroller:not([ios]) [part~="row"][dragstart]:not([dragstart=""])::after {
        ...
        `}constructor(){super(),this.lastAddress="",this.transactions=[],this.lastBlock={height:0},this.addressesInfo={},this.selectedAddress={},this.selectedAddressInfo={nativeBalance:{total:{}},transactions:[]},this.addressesUnconfirmedTransactions={},this.addressInfoStreams={},this.unconfirmedTransactionStreams={},this.selectedTransaction={},this.isTextMenuOpen=!1,gd.ready().then(()=>{gd.subscribe("selected_address",async e=>{this.selectedAddress={},(e=JSON.parse(e))&&0!==Object.entries(e).length&&(this.selectedAddress=e,this.updateAccountTransactions(),this.updateAccountBalance())}),gd.subscribe("copy_menu_switch",async e=>{"false"===e&&!0===this.isTextMenuOpen&&(this.clearSelection(),this.isTextMenuOpen=!1)})})}render(){return L`
            <div class="white-bg">
                    ...
                    <div id="contentDiv" style="margin: 8px;">
                        ...
                        <div class="layout horizontal">
                            ...
                            <vaadin-grid id="transactionsGrid" style="height:auto;" ?hidden="${this.isEmptyArray(this.transactions)}" aria-label="Peers" .items="${this.transactions}" height-by-rows>
                                <vaadin-grid-column width="4.4rem" header="Type" .renderer=${(e,t,i)=>{M(L`
                                ${i.item.type} 
                                    ${i.item.creatorAddress===this.selectedAddress.address?L`<span class="color-out">OUT</span>`:L`<span class="color-in">IN</span>`}
                ...
                `,e)}}>
                                ...
                                <vaadin-grid-column width="2rem" header="Timestamp" .renderer=${(e,t,i)=>{const r=new Date(i.item.timestamp);M(L`
        ...
        `}getGridTransaction(){const e=this.shadowRoot.querySelector("#transactionsGrid");e.addEventListener("click",t=>{let i=e.getEventContext(t).item;this.showTransactionDetails(i,this.transactions)},{passive:!0})}_textMenu(e){const t=()=>{var e="";return void 0!==window.getSelection?e=window.getSelection().toString():void 0!==this.shadowRoot.selection&&"Text"==this.shadowRoot.selection.type&&(e=this.shadowRoot.selection.createRange().text),e};(()=>{let i=t();if(i&&"string"==typeof i){let t={selectedText:i,eventObject:{pageX:e.pageX,pageY:e.pageY,clientX:e.clientX,clientY:e.clientY},isFrame:!0};gd.request("openCopyTextMenu",t)}})()}clearSelection(){window.getSelection().removeAllRanges(),window.parent.getSelection().removeAllRanges()}firstUpdated(){this.getGridTransaction(),window.addEventListener("contextmenu",e=>{e.preventDefault(),this.isTextMenuOpen=!0,this._textMenu(e)}),window.addEventListener("click",()=>{this.isTextMenuOpen&&gd.request("closeCopyTextMenu",null)}),window.onkeyup=e=>{27===e.keyCode&&gd.request("closeCopyTextMenu",null)}}updateAccountTransactions(){clearTimeout(this.updateAccountTransactionTimeout),gd.request("apiCall",{url:`/transactions/search?address=${this.selectedAddress.address}&confirmationStatus=BOTH&limit=20&reverse=true`}).then(e=>{this.transactions=e,this.updateAccountTransactionTimeout=setTimeout(()=>this.updateAccountTransactions(),5e3)})}updateAccountInfo(){clearTimeout(this.updateAccountInfoTimeout),gd.request("apiCall",{url:"/addresses/"+this.selectedAddress.address}).then(e=>{this.addressInfo=e,this.updateAccountInfoTimeout=setTimeout(()=>this.updateAccountInfo(),4e3)})}updateAccountBalance(){clearTimeout(this.updateAccountBalanceTimeout),gd.request("apiCall",{url:"/addresses/balance/"+this.selectedAddress.address}).then(e=>{this.balance=e,this.updateAccountBalanceTimeout=setTimeout(()=>this.updateAccountBalance(),5e3)})}showTransactionDetails(e,t){t.forEach(t=>{if(e.signature===t.signature){let i=e.creatorAddress===this.selectedAddress.address?"OUT":"IN";this.selectedTransaction={...t,txnFlow:i},0!=this.selectedTransaction.signature.length&&this.shadowRoot.querySelector("#showTransactionDetailsDialog").show()}})}isEmptyArray(e){return!e||0===e.length}floor(e){return e=parseFloat(e),isNaN(e)?0:this._format(Math.floor(e))}decimals(e){return(e=parseFloat(e))%1>0?(e+"").split(".")[1]:"0"}sendOrRecieve(e){return e.sender==this.selectedAddress.address}senderOrRecipient(e){return this.sendOrRecieve(e)?e.recipient:e.sender}txColor(e){return this.sendOrRecieve(e)?"red":"green"}getTxType(e){return fd[e]}subtract(e,t){return e-t}getConfirmations(e,t){return t-e+1}_format(e){return e.toLocaleString()}textColor(e){return"light"===e?"rgba(255,255,255,0.7)":"rgba(0,0,0,0.87)"}_unconfirmedClass(e){return e?"unconfirmed":""}})}));
