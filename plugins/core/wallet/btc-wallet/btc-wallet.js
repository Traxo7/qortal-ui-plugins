...
`);const _l=/^((?!chrome|android).)*safari/i.test(navigator.userAgent),fl=navigator.userAgent.toLowerCase().indexOf("firefox")>-1;if(_l||fl){const e=document.createElement("style");e.textContent="\n    [scrolling][safari] #outerscroller,\n    [scrolling][firefox] #outerscroller {\n      pointer-events: auto;\n    }\n\n    [ios] #outerscroller {\n      pointer-events: auto;\n      z-index: -3;\n    }\n\n    [ios][scrolling] #outerscroller {\n      z-index: 0;\n    }\n\n    [ios] [frozen] {\n      will-change: auto;\n    }\n  ",ml.querySelector("template").content.appendChild(e)}ml.register("vaadin-grid-styles");const gl=(()=>{try{return document.createEvent("TouchEvent"),!0}catch(e){return!1}})();class bl extends(Ta(ha(ja(Ga(qa(Va(Xa(Za(Qa(Ka(dl(Ba(Wa(cl(Ua(Ya(ll(el(Ha))))))))))))))))))){static get template(){return Bs`
  ...
  <template>
    <style>
      ...
      :host([dir="rtl"]) #scroller:not([ios]) [part~="row"][dragstart]:not([dragstart=""])::after {
        ...
        `}constructor(){super(),this.transactions=[],this.lastBlock={height:0},this.selectedBtcWallet={},this.balance=0,this.selectedTransaction={},this.isTextMenuOpen=!1,fd.ready().then(()=>{fd.subscribe("selected_address",async e=>{this.selectedBtcWallet={},(e=JSON.parse(e))&&0!==Object.entries(e).length&&(this.selectedBtcWallet=e.btcWallet)}),fd.subscribe("copy_menu_switch",async e=>{"false"===e&&!0===this.isTextMenuOpen&&(this.clearSelection(),this.isTextMenuOpen=!1)})})}render(){return L`
            <div class="white-bg">
                    ...
                    <div id="contentDiv" style="margin: 8px;">
                        <div class="layout horizontal">
                            ...
                            <vaadin-grid id="transactionsGrid" style="height:auto;" ?hidden="${this.isEmptyArray(this.transactions)}" aria-label="Peers" .items="${this.transactions}" height-by-rows>
                                ...
                                ${i.item.type} 
                                    ${i.item.creatorAddress===this.selectedBtcWallet.address?L`<span class="color-out">OUT</span>`:L`<span class="color-in">IN</span>`}
        ...
        `}getGridTransaction(){const e=this.shadowRoot.querySelector("#transactionsGrid");e.addEventListener("click",t=>{let i=e.getEventContext(t).item;this.showTransactionDetails(i,this.transactions)},{passive:!0})}_textMenu(e){const t=()=>{var e="";return void 0!==window.getSelection?e=window.getSelection().toString():void 0!==this.shadowRoot.selection&&"Text"==this.shadowRoot.selection.type&&(e=this.shadowRoot.selection.createRange().text),e};(()=>{let i=t();if(i&&"string"==typeof i){let t={selectedText:i,eventObject:{pageX:e.pageX,pageY:e.pageY,clientX:e.clientX,clientY:e.clientY},isFrame:!0};fd.request("openCopyTextMenu",t)}})()}clearSelection(){window.getSelection().removeAllRanges(),window.parent.getSelection().removeAllRanges()}firstUpdated(){this.getGridTransaction(),this.updateAccountBalance(),window.addEventListener("contextmenu",e=>{e.preventDefault(),this.isTextMenuOpen=!0,this._textMenu(e)}),window.addEventListener("click",()=>{this.isTextMenuOpen&&fd.request("closeCopyTextMenu",null)}),window.onkeyup=e=>{27===e.keyCode&&fd.request("closeCopyTextMenu",null)}}updateAccountBalance(){fd.request("apiCall",{url:"/crosschain/btc/walletbalance",method:"POST",body:window.parent.reduxStore.getState().app.selectedAddress.btcWallet.derivedMasterPrivateKey}).then(e=>{this.balance=(Number(e)/1e8).toFixed(8)}).catch(e=>{fd.request("showSnackBar","Failed to Fetch Balance. Try again!")})}showTransactionDetails(e,t){t.forEach(t=>{if(e.signature===t.signature){let i=e.creatorAddress===this.selectedBtcWallet.address?"OUT":"IN";this.selectedTransaction={...t,txnFlow:i},0!=this.selectedTransaction.signature.length&&this.shadowRoot.querySelector("#showTransactionDetailsDialog").show()}})}isEmptyArray(e){return!e||0===e.length}floor(e){return e=parseFloat(e),isNaN(e)?0:this._format(Math.floor(e))}decimals(e){return(e=parseFloat(e))%1>0?(e+"").split(".")[1]:"0"}sendOrRecieve(e){return e.sender==this.selectedBtcWallet.address}senderOrRecipient(e){return this.sendOrRecieve(e)?e.recipient:e.sender}txColor(e){return this.sendOrRecieve(e)?"red":"green"}subtract(e,t){return e-t}getConfirmations(e,t){return t-e+1}_format(e){return e.toLocaleString()}textColor(e){return"light"===e?"rgba(255,255,255,0.7)":"rgba(0,0,0,0.87)"}_unconfirmedClass(e){return e?"unconfirmed":""}})}));
